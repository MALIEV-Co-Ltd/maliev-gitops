...
2.  **Code Review & Merge:** The developer opens a Pull Request in the C# service's own repository (e.g., `Maliev.AuthService`). It is reviewed and merged into the `main` branch.
...
For each of your C# microservice repositories, perform the following steps:
...
3.  **Create GitHub Secrets**: In your C# repository's settings (`Settings > Secrets and variables > Actions`), create two new repository secrets:

...
- Verified `maliev-delivery-service-secrets` synced from `maliev-dev-delivery-service-config`, the pod reached Ready, `/delivery/liveness` returned 200, and `/delivery/readiness` returned Healthy with DeliveryDbContext, RabbitMQ, MassTransit, and IAM registration checks healthy.
...
Web, QuoteEngine, and Intranet BFF shipping calls all target DeliveryService through the shared service-client contract:

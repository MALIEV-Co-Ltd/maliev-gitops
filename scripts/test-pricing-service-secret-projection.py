#!/usr/bin/env python3
"""Adversarial tests for the PricingService GitOps projection policy."""
...
POLICY_PATH = REPOSITORY_ROOT / "scripts" / "check-pricing-service-secret-projection.py"
SPEC = importlib.util.spec_from_file_location("pricing_projection_policy", POLICY_PATH)
...
class PricingProjectionPolicyTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.original_pricing_root = POLICY.PRICING_ROOT
        POLICY.ROOT = self.root
        POLICY.PRICING_ROOT = self.root / "3-apps" / "maliev-pricing-service"

    def tearDown(self) -> None:
        POLICY.ROOT = self.original_root
        POLICY.PRICING_ROOT = self.original_pricing_root
        self.temporary_directory.cleanup()

    def copy_pricing_manifests(self) -> None:
        """Copy the current Pricing manifests and shared empty transformer."""
        shutil.copytree(
            REPOSITORY_ROOT / "3-apps" / "maliev-pricing-service",
            self.root / "3-apps" / "maliev-pricing-service",
        )
    ...
    def copy_disabled_applications(self) -> None:
        """Copy only the exact disabled Pricing Application contracts."""
        for folder in ("dev", "staging", "prod"):
            ...
            shutil.copy2(
                ...
                / "_disabled_apps"
                / folder
                / "maliev-pricing-service.yaml",
                destination / "maliev-pricing-service.yaml",
            )
    ...
    def add_duplicate_env_patch(self, environment: str, relative_path: str) -> None:
        ...
        overlay = POLICY.PRICING_ROOT / "overlays" / environment
            ...
            yaml.safe_dump(
                {
                    "apiVersion": "apps/v1",
                    "kind": "Deployment",
                    "metadata": {"name": "maliev-pricing-service"},
                    "spec": {
                        "template": {
                            "spec": {
                                "containers": [
    ...
    def append_kustomization_field(
        ...
        kustomization_path = (
            POLICY.PRICING_ROOT / "overlays" / environment / "kustomization.yaml"
        )
        kustomization = yaml.safe_load(kustomization_path.read_text(encoding="utf-8"))
    ...
    def test_broad_env_from_and_data_from_are_rejected(self) -> None:
        ...
        external_secret["spec"]["dataFrom"] = [
            {"extract": {"key": "maliev-prod-pricing-service-config"}}
        ]
        ...
        self.assertTrue(any("forbidden PricingService projection" in error for error in errors))
    ...
    def test_legacy_path_json6902_secret_replace_restore_is_rejected(self) -> None:
        ...
        self.copy_pricing_manifests()
        overlay = POLICY.PRICING_ROOT / "overlays" / "production"
            ...
            yaml.safe_dump(
                [
                    ...
                    },
                    {
                        "op": "replace",
                        "path": "/spec/template/spec/containers/0/env/4/valueFrom/secretKeyRef/name",
                        "value": "maliev-pricing-service-secrets",
                    },
                ],
                sort_keys=False,
            ),
    ...
    def test_modern_name_only_externalsecret_replace_restore_is_rejected(self) -> None:
        ...
        self.append_kustomization_field(
            ...
            "patches",
            {
                "target": {"name": "maliev-pricing-service-secrets"},
                "patch": yaml.safe_dump(
                    [
                        {
                            "op": "replace",
    ...
    def test_service_account_token_projection_is_rejected(self) -> None:
        """PricingService does not use the Kubernetes API and must not receive its token."""
        self.copy_pricing_manifests()
        deployment_path = POLICY.PRICING_ROOT / "base" / "deployment.yaml"
        deployment = yaml.safe_load(deployment_path.read_text(encoding="utf-8"))
    ...
    def test_equivalent_repository_url_raw_child_is_rejected(self) -> None:
        ...
        self.write_application(
            ...
            "maliev-pricing-service-child",
            "3-apps/maliev-pricing-service/overlays/development",
        )

        errors = POLICY.validate_pricing_applications_remain_disabled()

        self.assertTrue(
            any("Application', 'maliev-pricing-service-child" in error for error in errors)
        )
...
kind: Deployment
metadata:
  name: encoded-pricing-runtime
spec:
  selector:
    matchLabels:
      app: encoded-pricing-runtime
  template:
    metadata:
      labels:
        app: encoded-pricing-runtime
    spec:
      containers:
        - name: runtime
          image: registry.example/maliev-pricing-service:review
""",
            encoding="utf-8",
        )
        ...
        self.assertTrue(any("encoded-pricing-runtime" in error for error in errors))
...
""",
        ...
        self.assertTrue(
            any("Deployment', 'maliev-pricing-service" in error for error in errors)
        )

    def test_renamed_workload_using_pricing_image_is_rejected(self) -> None:
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: worker
          image: asia-southeast1-docker.pkg.dev/maliev-website/maliev-website-artifact-dev/maliev-pricing-service:dev-review
""",
            encoding="utf-8",
        )
    ...
    def test_iam_origins_use_environment_external_https_api_routes(self) -> None:
        ...
        for environment, expected_origin in expected_origins.items():
            rendered = POLICY.render(POLICY.PRICING_ROOT / "overlays" / environment)
            deployment = next(
                document
    ...
    def test_deployment_replicas_and_hpa_drift_are_rejected(self) -> None:
        ...
        hpa_path = POLICY.PRICING_ROOT / "base" / "hpa.yaml"
    ...
    def test_dynamic_applicationset_activation_paths_are_rejected(self) -> None:
            ...
            yaml.safe_dump(
                {
                    "apiVersion": "argoproj.io/v1alpha1",
                    "kind": "ApplicationSet",
                    "metadata": {"name": "dynamic-pricing-root"},
                    "spec": {
                        "generators": [
                            {
                                "list": {
                                    "elements": [{"service": "pricing"}],
                                    "template": {
                                        "spec": {
                                            "source": {
                                                "path": "3-apps/maliev-pricing-service"
                                            }
                                        }
                                    },
                                }
        ...
        self.assertTrue(
            any("ApplicationSet can activate PricingService" in error for error in errors)
        )

    def test_runtime_image_detection_covers_every_container_set(self) -> None:
        """Main, init, and ephemeral Pricing images all identify the runtime."""
        pricing_image = "registry.example/maliev-pricing-service@sha256:review"
        for container_set in ("containers", "initContainers", "ephemeralContainers"):
            pod_spec = {
                "containers": [{"name": "main", "image": "example.invalid/generic"}],
                container_set: [{"name": "renamed", "image": pricing_image}],
            }
            workload = {
                "apiVersion": "apps/v1",
                "kind": "Deployment",
            ...
            with self.subTest(container_set=container_set):
                self.assertTrue(POLICY.workload_uses_pricing_runtime(workload))

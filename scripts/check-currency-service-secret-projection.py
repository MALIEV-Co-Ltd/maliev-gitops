#!/usr/bin/env python3
"""Validate CurrencyService's least-privilege runtime secret projection."""
...
CURRENCY_ROOT = ROOT / "3-apps" / "maliev-currency-service"
...
REQUIRED_SECRET_KEYS = {
    ...
    "ConnectionStrings__CurrencyDbContext",
...
EXPECTED_RENDERED_INVENTORY = Counter(
    {
        ("Service", "maliev-currency-service"): 1,
        ("Deployment", "maliev-currency-service"): 1,
        ("HorizontalPodAutoscaler", "maliev-currency-service-hpa"): 1,
        ("ExternalSecret", "maliev-currency-service-secrets"): 1,
        ("ServiceMonitor", "maliev-currency-service"): 1,
    }
)
CURRENCY_RENDERED_IDENTITIES = set(EXPECTED_RENDERED_INVENTORY)
POD_PRODUCING_KINDS = {
    "Pod",
    "Deployment",
    "StatefulSet",
...
def source_can_activate_currency(source: object) -> bool:
    """Fail closed when an Argo source directly or recursively reaches CurrencyService."""
    if not isinstance(source, dict):
        return False
    source_path = str(source.get("path", "")).strip("/")
    if source_path == ".":
        source_path = ""
    currency_path = "3-apps/maliev-currency-service"
    if "{{" in source_path or "}}" in source_path:
        return True
    if ".." in source_path.split("/"):
        return True
    if source_path == currency_path or source_path.startswith(f"{currency_path}/"):
        return True

    directory = source.get("directory")
    is_currency_ancestor = not source_path or currency_path.startswith(f"{source_path}/")
    if is_currency_ancestor and isinstance(directory, dict):
...
def image_is_currency_runtime(value: object) -> bool:
    """Match the canonical CurrencyService image regardless of registry, tag, or digest."""
    image = str(value or "").strip()
    if not image:
        return False
    image_name = image.rsplit("/", 1)[-1].split("@", 1)[0].split(":", 1)[0]
    return image_name.casefold() == "maliev-currency-service"


def workload_uses_currency_runtime(document: dict[str, object]) -> bool:
    """Detect Currency runtime identity even when workload resources are renamed."""
    ...
    if any(image_is_currency_runtime(container.get("image")) for container in containers):
        return True
    ...
    if "maliev-currency-service" in label_values:
        return True

    return "maliev-currency-service-secrets" in yaml.safe_dump(
        pod_spec, sort_keys=True
    )


def rendered_document_activates_currency(document: dict[str, object]) -> bool:
    """Return true when a rendered resource activates CurrencyService."""
    identity = (
        document.get("kind"),
        document.get("metadata", {}).get("name"),
    )
    if identity in CURRENCY_RENDERED_IDENTITIES:
        return True
    if workload_uses_currency_runtime(document):
        return True
    if document.get("kind") not in ("Application", "ApplicationSet"):
        return False
    names, sources = argo_sources(document)
    return any("maliev-currency-service" in name for name in names) or any(
        source_can_activate_currency(source) for source in sources
    )
...
def expected_external_secret(environment: str) -> dict[str, object]:
    ...
    currency_key = f"{ENVIRONMENT_PREFIXES[environment]}-currency-service-config"
    remote_keys = {
        ...
        "ConnectionStrings__CurrencyDbContext": currency_key,
        "ServiceAuthentication__ClientSecret": currency_key,
    }
    return {
...
def validate_currency_overlay(environment: str) -> list[str]:
    ...
    deployments = [
        ...
        and document.get("metadata", {}).get("name") == "maliev-currency-service"
    ]
    services = [
        document
        for document in documents
    ...
    external_secrets = [
        ...
        and document.get("metadata", {}).get("name")
        == "maliev-currency-service-secrets"
    ]
    if (
        len(deployments) != 1
        or len(services) != 1
        ...
        errors.append(
            f"{environment}: exactly one Currency Deployment, Service, HPA, and "
    ...
    expected_service_spec = {
        "type": "ClusterIP",
        "selector": {"app": "maliev-currency-service"},
    ...
    if [container.get("name") for container in containers] != ["maliev-currency-service"]:
    ...
    if container.get("envFrom"):
        errors.append(f"{environment}: CurrencyService must not use envFrom")
    ...
    expected_secret_references = {
        key: {"name": "maliev-currency-service-secrets", "key": key}
        for key in REQUIRED_SECRET_KEYS
    }
    ...
    for token in FORBIDDEN_TOKENS:
        if token in projection:
            errors.append(f"{environment}: forbidden CurrencyService projection {token!r}")
...
def validate_currency_applications_remain_disabled() -> list[str]:
    ...
    disabled_contracts = {
        "dev": {
            "name": "maliev-currency-service-dev",
            "environment": "development",
            "namespace": "maliev-dev",
            "revision": "main",
        },
        "staging": {
            "name": "maliev-currency-service-staging",
            "environment": "staging",
            "namespace": "maliev-staging",
            "revision": "main",
        },
        "prod": {
            "name": "maliev-currency-service-prod",
    ...
    for folder, contract in disabled_contracts.items():
        path = disabled / folder / "maliev-currency-service.yaml"
        if not path.is_file():
            errors.append(f"missing disabled CurrencyService Application: {path}")
            continue
        documents = [
            document
        ...
        if len(documents) != 1:
            errors.append(f"disabled Currency manifest must contain one Application: {path}")
            continue
        ...
        expected = {
            ...
            "name": contract["name"],
            "namespace": "argocd",
            "app": "maliev-currency-service",
            "environment": contract["environment"],
            "repoURL": GITOPS_REPOSITORY_URL,
            "revision": contract["revision"],
            "path": f"3-apps/maliev-currency-service/overlays/{contract['environment']}",
            "server": "https://kubernetes.default.svc",
            "destination": contract["namespace"],
        }
        if actual != expected:
            errors.append(f"disabled Currency Application contract is not exact: {path}")
    ...
    for manifest in [*argocd.rglob("*.yaml"), *argocd.rglob("*.yml")]:
        ...
        for document in yaml.safe_load_all(manifest.read_text(encoding="utf-8")):
            ...
            direct_reference = any("maliev-currency-service" in name for name in names) or any(
                source_can_activate_currency(source) for source in sources
            )
            ...
            if direct_reference or unresolved:
                errors.append(
                    f"active {kind} can activate CurrencyService: {manifest.relative_to(ROOT)}"
                )
        ...
        identities = sorted(
            {
                ...
                for document in documents
                if rendered_document_activates_currency(document)
            }
        )
        if identities:
            errors.append(
                f"active source {source_path!r} renders CurrencyService {identities!r}"
            )
    return errors


def main() -> int:
    """Run all CurrencyService GitOps policy checks."""
    errors: list[str] = []
    for environment in ENVIRONMENTS:
        try:
            errors.extend(validate_currency_overlay(environment))
        except (OSError, RuntimeError, yaml.YAMLError) as error:
            errors.append(str(error))
    errors.extend(validate_currency_applications_remain_disabled())
    if errors:
        print("CurrencyService secret projection policy failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print("CurrencyService secret projection policy passed for all environments.")
    return 0

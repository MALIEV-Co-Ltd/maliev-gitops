#!/usr/bin/env python3
"""Validate AccountingService's least-privilege runtime secret projection."""
...
ACCOUNTING_ROOT = ROOT / "3-apps" / "maliev-accounting-service"
...
def expected_probe_contracts(environment: str) -> dict[str, dict[str, object]]:
    ...
    return {
        ...
        "readinessProbe": {
            "httpGet": {"path": "/accounting/readiness", "port": 8080},
            **common,
            "successThreshold": 1,
        },
    }
...
REQUIRED_SECRET_KEYS = {
    ...
    "ConnectionStrings__AccountingDbContext",
...
EXPECTED_RENDERED_INVENTORY = Counter(
    {
        ("Service", "maliev-accounting-service"): 1,
        ("Deployment", "maliev-accounting-service"): 1,
        ("HorizontalPodAutoscaler", "maliev-accounting-service-hpa"): 1,
        ("ExternalSecret", "maliev-accounting-service-secrets"): 1,
        ("ServiceMonitor", "maliev-accounting-service"): 1,
    }
)
ACCOUNTING_RENDERED_IDENTITIES = set(EXPECTED_RENDERED_INVENTORY)
POD_PRODUCING_KINDS = {
    "Pod",
    "Deployment",
    "StatefulSet",
...
def source_can_activate_accounting(source: object) -> bool:
    """Fail closed when an Argo source directly or recursively reaches AccountingService."""
    if not isinstance(source, dict):
        return False
    source_path = str(source.get("path", "")).strip("/")
    if source_path == ".":
        source_path = ""
    accounting_path = "3-apps/maliev-accounting-service"
    if "{{" in source_path or "}}" in source_path:
        return True
    if ".." in source_path.split("/"):
        return True
    if source_path == accounting_path or source_path.startswith(f"{accounting_path}/"):
        return True

    directory = source.get("directory")
    is_accounting_ancestor = not source_path or accounting_path.startswith(f"{source_path}/")
    if is_accounting_ancestor and isinstance(directory, dict):
...
def image_is_accounting_runtime(value: object) -> bool:
    """Match the canonical AccountingService image regardless of registry, tag, or digest."""
    image = str(value or "").strip()
    if not image:
        return False
    image_name = image.rsplit("/", 1)[-1].split("@", 1)[0].split(":", 1)[0]
    return image_name.casefold() == "maliev-accounting-service"


def workload_uses_accounting_runtime(document: dict[str, object]) -> bool:
    """Detect Accounting runtime identity even when workload resources are renamed."""
    ...
    if any(image_is_accounting_runtime(container.get("image")) for container in containers):
        return True
    ...
    if "maliev-accounting-service" in label_values:
        return True

    return "maliev-accounting-service-secrets" in yaml.safe_dump(
        pod_spec, sort_keys=True
    )


def rendered_document_activates_accounting(document: dict[str, object]) -> bool:
    """Return true when a rendered resource activates AccountingService."""
    identity = (
        document.get("kind"),
        document.get("metadata", {}).get("name"),
    )
    if identity in ACCOUNTING_RENDERED_IDENTITIES:
        return True
    if workload_uses_accounting_runtime(document):
        return True
    if document.get("kind") not in ("Application", "ApplicationSet"):
        return False
    names, sources = argo_sources(document)
    return any("maliev-accounting-service" in name for name in names) or any(
        source_can_activate_accounting(source) for source in sources
    )
...
def expected_external_secret(environment: str) -> dict[str, object]:
    ...
    accounting_key = f"{ENVIRONMENT_PREFIXES[environment]}-accounting-service-config"
    remote_keys = {
        ...
        "ConnectionStrings__AccountingDbContext": accounting_key,
        "ServiceAuthentication__ClientSecret": accounting_key,
    }
    return {
...
def validate_source_environment_lists(environment: str) -> list[str]:
    ...
    source_documents, json_patches, errors = resolve_kustomize_source_documents(
        ACCOUNTING_ROOT / "overlays" / environment
    )
...
def validate_accounting_overlay(environment: str) -> list[str]:
    ...
    deployments = [
        ...
        and document.get("metadata", {}).get("name") == "maliev-accounting-service"
    ]
    services = [
        document
        for document in documents
    ...
    external_secrets = [
        ...
        and document.get("metadata", {}).get("name")
        == "maliev-accounting-service-secrets"
    ]
    if (
        len(deployments) != 1
        or len(services) != 1
        ...
        errors.append(
            f"{environment}: exactly one Accounting Deployment, Service, HPA, and "
    ...
    expected_service_spec = {
        "type": "ClusterIP",
        "selector": {"app": "maliev-accounting-service"},
    ...
    if [container.get("name") for container in containers] != ["maliev-accounting-service"]:
    ...
    if container.get("envFrom"):
        errors.append(f"{environment}: AccountingService must not use envFrom")
    if container.get("volumeMounts"):
        errors.append(f"{environment}: AccountingService volumeMounts are not allowlisted")
    ...
    expected_non_secret_values = {
        "ServiceAuthentication__ClientId": "service-accounting-service",
    ...
    expected_secret_references = {
        key: {"name": "maliev-accounting-service-secrets", "key": key}
        for key in REQUIRED_SECRET_KEYS
    }
    ...
    for token in FORBIDDEN_TOKENS:
        if token in projection:
            errors.append(f"{environment}: forbidden AccountingService projection {token!r}")
...
def validate_accounting_applications_remain_disabled() -> list[str]:
    ...
    disabled_contracts = {
        "dev": {
            "name": "maliev-accounting-service-dev",
            "environment": "development",
            "namespace": "maliev-dev",
            "revision": "main",
        },
        "staging": {
            "name": "maliev-accounting-service-staging",
            "environment": "staging",
            "namespace": "maliev-staging",
            "revision": "main",
        },
        "prod": {
            "name": "maliev-accounting-service-prod",
    ...
    for folder, contract in disabled_contracts.items():
        path = disabled / folder / "maliev-accounting-service.yaml"
        if not path.is_file():
            errors.append(f"missing disabled AccountingService Application: {path}")
            continue
        documents = [
            document
            for document in yaml.safe_load_all(path.read_text(encoding="utf-8"))
        ...
        if len(documents) != 1:
            errors.append(f"disabled Accounting manifest must contain one Application: {path}")
            continue
        ...
        expected = {
            ...
            "name": contract["name"],
            "namespace": "argocd",
            "app": "maliev-accounting-service",
            "environment": contract["environment"],
            "repoURL": GITOPS_REPOSITORY_URL,
            "revision": contract["revision"],
            "path": f"3-apps/maliev-accounting-service/overlays/{contract['environment']}",
            "server": "https://kubernetes.default.svc",
            "destination": contract["namespace"],
        }
        if actual != expected:
            errors.append(f"disabled Accounting Application contract is not exact: {path}")
    ...
    for manifest in [*argocd.rglob("*.yaml"), *argocd.rglob("*.yml")]:
        ...
        for document in yaml.safe_load_all(manifest.read_text(encoding="utf-8")):
            ...
            direct_reference = any("maliev-accounting-service" in name for name in names) or any(
                source_can_activate_accounting(source) for source in sources
            )
            ...
            if direct_reference or unresolved:
                errors.append(
                    f"active {kind} can activate AccountingService: {manifest.relative_to(ROOT)}"
                )
        ...
        identities = sorted(
            {
                ...
                for document in documents
                if rendered_document_activates_accounting(document)
            }
        )
        if identities:
            errors.append(
                f"active source {source_path!r} renders AccountingService {identities!r}"
            )
    return errors


def main() -> int:
    """Run all AccountingService GitOps policy checks."""
    errors: list[str] = []
    for environment in ENVIRONMENTS:
        try:
            errors.extend(validate_accounting_overlay(environment))
        except (OSError, RuntimeError, yaml.YAMLError) as error:
            errors.append(str(error))
    errors.extend(validate_accounting_applications_remain_disabled())
    if errors:
        print("AccountingService secret projection policy failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print("AccountingService secret projection policy passed for all environments.")
    return 0

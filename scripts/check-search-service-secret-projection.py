#!/usr/bin/env python3
"""Validate SearchService's least-privilege runtime secret projection."""
...
SEARCH_ROOT = ROOT / "3-apps" / "maliev-search-service"
...
EXPECTED_RENDERED_INVENTORY = Counter(
    {
        ("Service", "maliev-search-service"): 1,
        ("Deployment", "maliev-search-service"): 1,
        ("HorizontalPodAutoscaler", "maliev-search-service-hpa"): 1,
        ("ExternalSecret", "maliev-search-service-secrets"): 1,
        ("ServiceMonitor", "maliev-search-service"): 1,
    }
)
...
def source_can_activate_search(source: object) -> bool:
    """Fail closed when an Argo source directly or recursively reaches SearchService."""
    if not isinstance(source, dict):
        return False
    source_path = str(source.get("path", "")).strip("/")
    if source_path == ".":
        source_path = ""
    search_path = "3-apps/maliev-search-service"
    if "{{" in source_path or "}}" in source_path:
        return True
    if ".." in source_path.split("/"):
        return True
    if source_path == search_path or source_path.startswith(f"{search_path}/"):
        return True

    directory = source.get("directory")
    is_search_ancestor = not source_path or search_path.startswith(f"{source_path}/")
    if is_search_ancestor and isinstance(directory, dict):
...
def rendered_document_activates_search(document: dict[str, object]) -> bool:
    ...
    return any("maliev-search-service" in name for name in names) or any(
        source_can_activate_search(source) for source in sources
    )
...
def expected_external_secret(environment: str) -> dict[str, object]:
    ...
    search_key = f"{ENVIRONMENT_PREFIXES[environment]}-search-service-config"
    remote_keys = {
        ...
        "ConnectionStrings__SearchDbContext": search_key,
        "ServiceAuthentication__ClientSecret": search_key,
    }
    return {
        "apiVersion": "external-secrets.io/v1",
        "kind": "ExternalSecret",
        "metadata": {
            "name": "maliev-search-service-secrets",
...
def validate_search_overlay(environment: str) -> list[str]:
    """Validate exact rendered inventory, pod projection, and ExternalSecret."""
    errors: list[str] = []
    rendered = render(SEARCH_ROOT / "overlays" / environment)
    documents = [
    ...
    deployments = [
        ...
        and document.get("metadata", {}).get("name") == "maliev-search-service"
    ]
    external_secrets = [
        document
        for document in documents
        ...
        == "maliev-search-service-secrets"
    ]
    if len(deployments) != 1 or len(external_secrets) != 1:
        errors.append(
            f"{environment}: exactly one Search Deployment and ExternalSecret must render"
        )
        return errors
    ...
    if [container.get("name") for container in containers] != ["maliev-search-service"]:
    ...
    if container.get("envFrom"):
        errors.append(f"{environment}: SearchService must not use envFrom")
    if container.get("volumeMounts"):
        errors.append(f"{environment}: SearchService volumeMounts are not allowlisted")
    ...
    expected_secret_references = {
        key: {"name": "maliev-search-service-secrets", "key": key}
        for key in REQUIRED_SECRET_KEYS
    }
    ...
    for token in FORBIDDEN_TOKENS:
        if token in projection:
            errors.append(f"{environment}: forbidden SearchService projection {token!r}")
...
def validate_search_applications_remain_disabled() -> list[str]:
    ...
    disabled_contracts = {
        "dev": {
            "name": "maliev-search-service-dev",
            "environment": "development",
            "namespace": "maliev-dev",
            "revision": "main",
        },
        "staging": {
            "name": "maliev-search-service-staging",
            "environment": "staging",
            "namespace": "maliev-staging",
            "revision": "main",
        },
        "prod": {
            "name": "maliev-search-service-prod",
    ...
    for folder, contract in disabled_contracts.items():
        path = disabled / folder / "maliev-search-service.yaml"
        if not path.is_file():
            errors.append(f"missing disabled SearchService Application: {path}")
            continue
        documents = [
            document
        ...
        if len(documents) != 1:
            errors.append(f"disabled Search manifest must contain one Application: {path}")
            continue
        ...
        expected = {
            ...
            "name": contract["name"],
            "namespace": "argocd",
            "app": "maliev-search-service",
            "environment": contract["environment"],
            "repoURL": GITOPS_REPOSITORY_URL,
            "revision": contract["revision"],
            "path": f"3-apps/maliev-search-service/overlays/{contract['environment']}",
            "server": "https://kubernetes.default.svc",
            "destination": contract["namespace"],
        }
        if actual != expected:
            errors.append(f"disabled Search Application contract is not exact: {path}")
    ...
    for manifest in [*argocd.rglob("*.yaml"), *argocd.rglob("*.yml")]:
        ...
        for document in yaml.safe_load_all(manifest.read_text(encoding="utf-8")):
            ...
            direct_reference = any("maliev-search-service" in name for name in names) or any(
                source_can_activate_search(source) for source in sources
            )
            ...
            if direct_reference or unresolved:
                errors.append(
                    f"active {kind} can activate SearchService: {manifest.relative_to(ROOT)}"
                )
        ...
        identities = sorted(
            {
                ...
                for document in documents
                if rendered_document_activates_search(document)
            }
        )
        if identities:
            errors.append(
                f"active source {source_path!r} renders SearchService {identities!r}"
            )
    return errors


def main() -> int:
    """Run all SearchService GitOps policy checks."""
    errors: list[str] = []
    for environment in ENVIRONMENTS:
        try:
            errors.extend(validate_search_overlay(environment))
        except (OSError, RuntimeError, yaml.YAMLError) as error:
            errors.append(str(error))
    errors.extend(validate_search_applications_remain_disabled())
    if errors:
        print("SearchService secret projection policy failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print("SearchService secret projection policy passed for all environments.")
    return 0

#!/usr/bin/env python3
"""Adversarial tests for the MaterialService GitOps projection policy."""
...
POLICY_PATH = REPOSITORY_ROOT / "scripts" / "check-material-service-secret-projection.py"
SPEC = importlib.util.spec_from_file_location("material_projection_policy", POLICY_PATH)
...
class MaterialProjectionPolicyTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.original_material_root = POLICY.MATERIAL_ROOT
        POLICY.ROOT = self.root
        POLICY.MATERIAL_ROOT = self.root / "3-apps" / "maliev-material-service"

    def tearDown(self) -> None:
        POLICY.ROOT = self.original_root
        POLICY.MATERIAL_ROOT = self.original_material_root
        self.temporary_directory.cleanup()

    def copy_material_manifests(self) -> None:
        """Copy the current Material manifests and shared empty transformer."""
        shutil.copytree(
            REPOSITORY_ROOT / "3-apps" / "maliev-material-service",
            self.root / "3-apps" / "maliev-material-service",
        )
    ...
    def copy_disabled_applications(self) -> None:
        """Copy only the exact disabled Material Application contracts."""
        for folder in ("dev", "staging", "prod"):
            ...
            shutil.copy2(
                ...
                destination / "maliev-material-service.yaml",
            )
    ...
    def add_duplicate_env_patch(self, environment: str, relative_path: str) -> None:
        ...
        overlay = POLICY.MATERIAL_ROOT / "overlays" / environment
            ...
            yaml.safe_dump(
                {
                    "apiVersion": "apps/v1",
                    "kind": "Deployment",
                    "metadata": {"name": "maliev-material-service"},
                    "spec": {
                        "template": {
                            "spec": {
                                "containers": [
    ...
    def append_kustomization_field(
        ...
        kustomization_path = (
            POLICY.MATERIAL_ROOT / "overlays" / environment / "kustomization.yaml"
        )
        kustomization = yaml.safe_load(kustomization_path.read_text(encoding="utf-8"))
    ...
    def test_broad_env_from_and_data_from_are_rejected(self) -> None:
        ...
        self.copy_material_manifests()
        deployment_path = POLICY.MATERIAL_ROOT / "base" / "deployment.yaml"
        deployment = yaml.safe_load(deployment_path.read_text(encoding="utf-8"))
        container = deployment["spec"]["template"]["spec"]["containers"][0]
        container["envFrom"] = [
            {"secretRef": {"name": "maliev-shared-secrets"}},
            {"secretRef": {"name": "maliev-material-service-secrets"}},
        ]
        deployment_path.write_text(
            yaml.safe_dump(deployment, sort_keys=False), encoding="utf-8"
        )

        secret_path = POLICY.MATERIAL_ROOT / "base" / "service-secrets.yaml"
        ...
        external_secret["spec"]["dataFrom"] = [
            {"extract": {"key": "maliev-prod-material-service-config"}}
        ]
        secret_path.write_text(
            yaml.safe_dump(external_secret, sort_keys=False), encoding="utf-8"
        )

        errors = POLICY.validate_material_overlay("production")

        self.assertTrue(any("must not use envFrom" in error for error in errors))
        self.assertTrue(any("forbidden MaterialService projection" in error for error in errors))
        self.assertTrue(any("exact object contract" in error for error in errors))

    def test_hmac_and_service_signing_material_are_rejected(self) -> None:
        """A verifier-only service must never receive symmetric or signing material."""
        self.copy_material_manifests()
        deployment_path = POLICY.MATERIAL_ROOT / "base" / "deployment.yaml"
        deployment = yaml.safe_load(deployment_path.read_text(encoding="utf-8"))
        container = deployment["spec"]["template"]["spec"]["containers"][0]
        container["env"].extend(
            [
                {"name": "Jwt__HmacKey", "value": "forbidden"},
                {
                    "name": "ServiceAuthentication__SigningKey",
                    "value": "forbidden",
                },
            ]
        )
        ...
        self.assertTrue(any("Jwt__HmacKey" in error for error in errors))
        self.assertTrue(
            any("ServiceAuthentication__SigningKey" in error for error in errors)
        )
    ...
    def test_legacy_path_json6902_secret_replace_restore_is_rejected(self) -> None:
        ...
        self.copy_material_manifests()
        overlay = POLICY.MATERIAL_ROOT / "overlays" / "production"
            ...
            yaml.safe_dump(
                [
                    ...
                    },
                    {
                        "op": "replace",
                        "path": "/spec/template/spec/containers/0/env/4/valueFrom/secretKeyRef/name",
                        "value": "maliev-material-service-secrets",
                    },
                ],
                sort_keys=False,
            ),
    ...
    def test_modern_name_only_externalsecret_replace_restore_is_rejected(self) -> None:
        ...
        self.append_kustomization_field(
            ...
            "patches",
            {
                "target": {"name": "maliev-material-service-secrets"},
                "patch": yaml.safe_dump(
                    [
                        {
                            "op": "replace",
    ...
    def test_service_account_token_projection_is_rejected(self) -> None:
        """MaterialService does not use the Kubernetes API and must not receive its token."""
        self.copy_material_manifests()
        deployment_path = POLICY.MATERIAL_ROOT / "base" / "deployment.yaml"
        deployment = yaml.safe_load(deployment_path.read_text(encoding="utf-8"))
    ...
    def test_equivalent_repository_url_raw_child_is_rejected(self) -> None:
        ...
        self.write_application(
            self.root / "raw-root" / "child.yaml",
            "maliev-material-service-child",
            "3-apps/maliev-material-service/overlays/development",
        )

        errors = POLICY.validate_material_applications_remain_disabled()

        self.assertTrue(
            any("Application', 'maliev-material-service-child" in error for error in errors)
        )
...
kind: Deployment
metadata:
  name: encoded-material-runtime
spec:
  selector:
    matchLabels:
      app: encoded-material-runtime
  template:
    metadata:
      labels:
        app: encoded-material-runtime
    spec:
      containers:
        - name: runtime
          image: registry.example/maliev-material-service:review
""",
            encoding="utf-8",
        )
        ...
        self.assertTrue(any("encoded-material-runtime" in error for error in errors))
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: material
          image: example.invalid/material
""",
            encoding="utf-8",
        )
        ...
        self.assertTrue(
            any("Deployment', 'maliev-material-service" in error for error in errors)
        )

    def test_renamed_workload_using_material_image_is_rejected(self) -> None:
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: worker
          image: asia-southeast1-docker.pkg.dev/maliev-website/maliev-website-artifact-dev/maliev-material-service:dev-review
""",
            encoding="utf-8",
        )
    ...
    def test_iam_origins_use_environment_external_https_api_routes(self) -> None:
        ...
        for environment, expected_origin in expected_origins.items():
            rendered = POLICY.render(POLICY.MATERIAL_ROOT / "overlays" / environment)
            deployment = next(
                document
    ...
    def test_servicemonitor_drift_is_rejected(self) -> None:
        ...
        self.copy_material_manifests()
        monitor_path = POLICY.MATERIAL_ROOT / "base" / "servicemonitor.yaml"
    ...
    def test_deployment_replicas_and_hpa_drift_are_rejected(self) -> None:
        ...
        hpa_path = POLICY.MATERIAL_ROOT / "base" / "hpa.yaml"
    ...
    def test_dynamic_applicationset_activation_paths_are_rejected(self) -> None:
            ...
            yaml.safe_dump(
                {
                    "apiVersion": "argoproj.io/v1alpha1",
                    "kind": "ApplicationSet",
                    "metadata": {"name": "dynamic-material-root"},
                    "spec": {
                        "generators": [
                            {
                                "list": {
                                    "elements": [{"service": "material"}],
                                    "template": {
                                        "spec": {
                                            "source": {
                                                "path": "3-apps/maliev-material-service"
                                            }
                                        }
                                    },
                                }
        ...
        self.assertTrue(
            any("ApplicationSet can activate MaterialService" in error for error in errors)
        )

    def test_runtime_image_detection_covers_every_container_set(self) -> None:
        """Main, init, and ephemeral Material images all identify the runtime."""
        material_image = "registry.example/maliev-material-service@sha256:review"
        for container_set in ("containers", "initContainers", "ephemeralContainers"):
            pod_spec = {
                "containers": [{"name": "main", "image": "example.invalid/generic"}],
                container_set: [{"name": "renamed", "image": material_image}],
            }
            workload = {
                "apiVersion": "apps/v1",
                "kind": "Deployment",
            ...
            with self.subTest(container_set=container_set):
                self.assertTrue(POLICY.workload_uses_material_runtime(workload))

#!/usr/bin/env python3
"""Validate MaterialService's least-privilege runtime secret projection."""
...
MATERIAL_ROOT = ROOT / "3-apps" / "maliev-material-service"
...
EXPECTED_ENVIRONMENT_VALUES = {
    "development": {
        ...
        "Services__SupplierService__BaseUrl": "https://dev.api.maliev.com",
    },
    "staging": {
        ...
        "Services__SupplierService__BaseUrl": "https://staging.api.maliev.com",
    },
    "production": {
        "ASPNETCORE_ENVIRONMENT": "Production",
        "Services__AuthService__BaseUrl": "https://api.maliev.com",
        "Services__IAMService__BaseUrl": "https://api.maliev.com",
        "Services__SupplierService__BaseUrl": "https://api.maliev.com",
    },
}
EXPECTED_HPA_BOUNDS = {
    "development": (1, 1),
...
def expected_probe_contracts(environment: str) -> dict[str, dict[str, object]]:
    ...
    return {
        "livenessProbe": {
            "httpGet": {"path": "/material/liveness", "port": 8080},
            **common,
        },
        "readinessProbe": {
            "httpGet": {"path": "/material/readiness", "port": 8080},
            **common,
            "successThreshold": 1,
        },
    }
...
REQUIRED_SECRET_KEYS = {
    ...
    "ConnectionStrings__MaterialDbContext",
...
FORBIDDEN_TOKENS = (
    ...
    "Jwt__HmacKey",
    "Jwt__HMACKey",
    "ServiceAuthentication__SigningKey",
)
EXPECTED_RENDERED_INVENTORY = Counter(
    {
        ("Service", "maliev-material-service"): 1,
        ("Deployment", "maliev-material-service"): 1,
        ("HorizontalPodAutoscaler", "maliev-material-service-hpa"): 1,
        ("ExternalSecret", "maliev-material-service-secrets"): 1,
        ("ServiceMonitor", "maliev-material-service"): 1,
    }
)
MATERIAL_RENDERED_IDENTITIES = set(EXPECTED_RENDERED_INVENTORY)
POD_PRODUCING_KINDS = {
    "Pod",
    "Deployment",
    "StatefulSet",
...
def source_can_activate_material(source: object) -> bool:
    """Fail closed when an Argo source directly or recursively reaches MaterialService."""
    if not isinstance(source, dict):
        return False
    source_path = str(source.get("path", "")).strip("/")
    if source_path == ".":
        source_path = ""
    material_path = "3-apps/maliev-material-service"
    if "{{" in source_path or "}}" in source_path:
        return True
    if ".." in source_path.split("/"):
        return True
    if source_path == material_path or source_path.startswith(f"{material_path}/"):
        return True

    directory = source.get("directory")
    is_material_ancestor = not source_path or material_path.startswith(f"{source_path}/")
    if is_material_ancestor and isinstance(directory, dict):
...
def image_is_material_runtime(value: object) -> bool:
    """Match the canonical MaterialService image regardless of registry, tag, or digest."""
    image = str(value or "").strip()
    if not image:
        return False
    image_name = image.rsplit("/", 1)[-1].split("@", 1)[0].split(":", 1)[0]
    return image_name.casefold() == "maliev-material-service"


def workload_uses_material_runtime(document: dict[str, object]) -> bool:
    """Detect Material runtime identity even when workload resources are renamed."""
    ...
    if any(image_is_material_runtime(container.get("image")) for container in containers):
        return True
    ...
    if "maliev-material-service" in label_values:
        return True

    return "maliev-material-service-secrets" in yaml.safe_dump(
        pod_spec, sort_keys=True
    )


def rendered_document_activates_material(document: dict[str, object]) -> bool:
    """Return true when a rendered resource activates MaterialService."""
    identity = (
        document.get("kind"),
        document.get("metadata", {}).get("name"),
    )
    if identity in MATERIAL_RENDERED_IDENTITIES:
        return True
    if workload_uses_material_runtime(document):
        return True
    if document.get("kind") not in ("Application", "ApplicationSet"):
        return False
    names, sources = argo_sources(document)
    return any("maliev-material-service" in name for name in names) or any(
        source_can_activate_material(source) for source in sources
    )
...
def expected_external_secret(environment: str) -> dict[str, object]:
    ...
    material_key = f"{ENVIRONMENT_PREFIXES[environment]}-material-service-config"
    remote_keys = {
        ...
        "ConnectionStrings__MaterialDbContext": material_key,
        "ServiceAuthentication__ClientSecret": material_key,
    }
    return {
        "apiVersion": "external-secrets.io/v1",
        "kind": "ExternalSecret",
        "metadata": {
            "name": "maliev-material-service-secrets",
...
def expected_service_monitor(environment: str) -> dict[str, object]:
    ...
    return {
        ...
        "spec": {
            "selector": {
                "matchLabels": {"app": "maliev-material-service"},
            },
            "endpoints": [
                {
                    "port": "http",
                    "path": "/material/metrics",
                    "interval": "30s",
                    "scrapeTimeout": "10s",
                }
            ],
...
def validate_source_environment_lists(environment: str) -> list[str]:
    ...
    source_documents, json_patches, errors = resolve_kustomize_source_documents(
        MATERIAL_ROOT / "overlays" / environment
    )
...
def validate_material_overlay(environment: str) -> list[str]:
    ...
    deployments = [
        ...
        and document.get("metadata", {}).get("name") == "maliev-material-service"
    ]
    services = [
        document
        for document in documents
    ...
    external_secrets = [
        ...
        and document.get("metadata", {}).get("name")
        == "maliev-material-service-secrets"
    ]
    service_monitors = [
        document
        for document in documents
    ...
    if (
        ...
        errors.append(
            f"{environment}: exactly one Material Deployment, Service, HPA, and "
    ...
    expected_service_spec = {
        "type": "ClusterIP",
        "selector": {"app": "maliev-material-service"},
    ...
    if [container.get("name") for container in containers] != ["maliev-material-service"]:
    ...
    if container.get("envFrom"):
        errors.append(f"{environment}: MaterialService must not use envFrom")
    if container.get("volumeMounts"):
        errors.append(f"{environment}: MaterialService volumeMounts are not allowlisted")
    ...
    expected_non_secret_values = {
        "ServiceAuthentication__ClientId": "service-material-service",
    ...
    expected_secret_references = {
        key: {"name": "maliev-material-service-secrets", "key": key}
        for key in REQUIRED_SECRET_KEYS
    }
    ...
    for token in FORBIDDEN_TOKENS:
        if token in projection:
            errors.append(f"{environment}: forbidden MaterialService projection {token!r}")
...
def validate_material_applications_remain_disabled() -> list[str]:
    ...
    for folder, contract in disabled_contracts.items():
        path = disabled / folder / "maliev-material-service.yaml"
        if not path.is_file():
            errors.append(f"missing disabled MaterialService Application: {path}")
            continue
        documents = [
            document
            for document in yaml.safe_load_all(path.read_text(encoding="utf-8"))
        ...
        if len(documents) != 1:
            errors.append(f"disabled Material manifest must contain one Application: {path}")
            continue
    ...
    for manifest in [*argocd.rglob("*.yaml"), *argocd.rglob("*.yml")]:
        ...
        for document in yaml.safe_load_all(manifest.read_text(encoding="utf-8")):
            ...
            direct_reference = any("maliev-material-service" in name for name in names) or any(
                source_can_activate_material(source) for source in sources
            )
            ...
            if direct_reference or unresolved:
                errors.append(
                    f"active {kind} can activate MaterialService: {manifest.relative_to(ROOT)}"
                )
        ...
        identities = sorted(
            {
                ...
                for document in documents
                if rendered_document_activates_material(document)
            }
        )
        if identities:
            errors.append(
                f"active source {source_path!r} renders MaterialService {identities!r}"
            )
    return errors


def main() -> int:
    """Run all MaterialService GitOps policy checks."""
    errors: list[str] = []
    for environment in ENVIRONMENTS:
        try:
            errors.extend(validate_material_overlay(environment))
        except (OSError, RuntimeError, yaml.YAMLError) as error:
            errors.append(str(error))
    errors.extend(validate_material_applications_remain_disabled())
    if errors:
        print("MaterialService secret projection policy failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print("MaterialService secret projection policy passed for all environments.")
    return 0

#!/usr/bin/env python3
"""Validate RegistryService's least-privilege runtime secret projection."""
...
REGISTRY_ROOT = ROOT / "3-apps" / "maliev-registry-service"
...
REQUIRED_SECRET_KEYS = {
    ...
    "ConnectionStrings__RegistryDbContext",
    "BDEX__ConsumerKey",
    "BDEX__ConsumerSecret",
...
EXPECTED_RENDERED_INVENTORY = Counter(
    {
        ("Service", "maliev-registry-service"): 1,
        ("Deployment", "maliev-registry-service"): 1,
        ("HorizontalPodAutoscaler", "maliev-registry-service-hpa"): 1,
        ("ExternalSecret", "maliev-registry-service-secrets"): 1,
        ("ServiceMonitor", "maliev-registry-service"): 1,
    }
)
REGISTRY_RENDERED_IDENTITIES = set(EXPECTED_RENDERED_INVENTORY)
POD_PRODUCING_KINDS = {
    "Pod",
    "Deployment",
    "StatefulSet",
...
def source_can_activate_registry(source: object) -> bool:
    """Fail closed when an Argo source directly or recursively reaches RegistryService."""
    if not isinstance(source, dict):
        return False
    source_path = str(source.get("path", "")).strip("/")
    if source_path == ".":
        source_path = ""
    registry_path = "3-apps/maliev-registry-service"
    if "{{" in source_path or "}}" in source_path:
        return True
    if ".." in source_path.split("/"):
        return True
    if source_path == registry_path or source_path.startswith(f"{registry_path}/"):
        return True

    directory = source.get("directory")
    is_registry_ancestor = not source_path or registry_path.startswith(f"{source_path}/")
    if is_registry_ancestor and isinstance(directory, dict):
...
def image_is_registry_runtime(value: object) -> bool:
    """Match the canonical RegistryService image regardless of registry, tag, or digest."""
    image = str(value or "").strip()
    if not image:
        return False
    image_name = image.rsplit("/", 1)[-1].split("@", 1)[0].split(":", 1)[0]
    return image_name.casefold() == "maliev-registry-service"


def workload_uses_registry_runtime(document: dict[str, object]) -> bool:
    """Detect Registry runtime identity even when workload resources are renamed."""
    ...
    if any(image_is_registry_runtime(container.get("image")) for container in containers):
        return True
    ...
    if "maliev-registry-service" in label_values:
        return True

    return "maliev-registry-service-secrets" in yaml.safe_dump(
        pod_spec, sort_keys=True
    )


def rendered_document_activates_registry(document: dict[str, object]) -> bool:
    """Return true when a rendered resource activates RegistryService."""
    identity = (
        document.get("kind"),
        document.get("metadata", {}).get("name"),
    )
    if identity in REGISTRY_RENDERED_IDENTITIES:
        return True
    if workload_uses_registry_runtime(document):
        return True
    if document.get("kind") not in ("Application", "ApplicationSet"):
        return False
    names, sources = argo_sources(document)
    return any("maliev-registry-service" in name for name in names) or any(
        source_can_activate_registry(source) for source in sources
    )
...
def expected_external_secret(environment: str) -> dict[str, object]:
    ...
    registry_key = f"{ENVIRONMENT_PREFIXES[environment]}-registry-service-config"
    remote_keys = {
        ...
        "ConnectionStrings__RegistryDbContext": registry_key,
        "ServiceAuthentication__ClientSecret": registry_key,
        "BDEX__ConsumerKey": registry_key,
        "BDEX__ConsumerSecret": registry_key,
    }
    return {
        "apiVersion": "external-secrets.io/v1",
        "kind": "ExternalSecret",
        "metadata": {
            "name": "maliev-registry-service-secrets",
...
def validate_registry_overlay(environment: str) -> list[str]:
    """Validate exact rendered inventory, pod projection, and ExternalSecret."""
    errors: list[str] = []
    rendered = render(REGISTRY_ROOT / "overlays" / environment)
    documents = [
    ...
    deployments = [
        ...
        and document.get("metadata", {}).get("name") == "maliev-registry-service"
    ]
    external_secrets = [
        document
        for document in documents
    ...
    if len(deployments) != 1 or len(external_secrets) != 1:
        errors.append(
            f"{environment}: exactly one Registry Deployment and ExternalSecret must render"
        )
        return errors
    ...
    if [container.get("name") for container in containers] != ["maliev-registry-service"]:
    ...
    if container.get("envFrom"):
        errors.append(f"{environment}: RegistryService must not use envFrom")
    if container.get("volumeMounts"):
        errors.append(f"{environment}: RegistryService volumeMounts are not allowlisted")

    environment_variables = {
        item.get("name"): item for item in container.get("env", []) if item.get("name")
    }
    expected_non_secret_values = {
        "ServiceAuthentication__ClientId": "service-registry-service",
    ...
    expected_secret_references = {
        key: {"name": "maliev-registry-service-secrets", "key": key}
        for key in REQUIRED_SECRET_KEYS
    }
    ...
    for token in FORBIDDEN_TOKENS:
        if token in projection:
            errors.append(f"{environment}: forbidden RegistryService projection {token!r}")
...
def validate_registry_applications_remain_disabled() -> list[str]:
    ...
    disabled_contracts = {
        "dev": {
            "name": "maliev-registry-service-dev",
            "environment": "development",
            "namespace": "maliev-dev",
            "revision": "main",
        },
        "staging": {
            "name": "maliev-registry-service-staging",
            "environment": "staging",
            "namespace": "maliev-staging",
            "revision": "main",
        },
        "prod": {
            "name": "maliev-registry-service-prod",
    ...
    for folder, contract in disabled_contracts.items():
        path = disabled / folder / "maliev-registry-service.yaml"
        if not path.is_file():
            errors.append(f"missing disabled RegistryService Application: {path}")
            continue
        documents = [
            document
        ...
        if len(documents) != 1:
            errors.append(f"disabled Registry manifest must contain one Application: {path}")
            continue
        ...
        expected = {
            ...
            "name": contract["name"],
            "namespace": "argocd",
            "app": "maliev-registry-service",
            "environment": contract["environment"],
            "repoURL": GITOPS_REPOSITORY_URL,
            "revision": contract["revision"],
            "path": f"3-apps/maliev-registry-service/overlays/{contract['environment']}",
            "server": "https://kubernetes.default.svc",
            "destination": contract["namespace"],
        }
        if actual != expected:
            errors.append(f"disabled Registry Application contract is not exact: {path}")
    ...
    for manifest in [*argocd.rglob("*.yaml"), *argocd.rglob("*.yml")]:
        ...
        for document in yaml.safe_load_all(manifest.read_text(encoding="utf-8")):
            ...
            direct_reference = any("maliev-registry-service" in name for name in names) or any(
                source_can_activate_registry(source) for source in sources
            )
            ...
            if direct_reference or unresolved:
                errors.append(
                    f"active {kind} can activate RegistryService: {manifest.relative_to(ROOT)}"
                )
        ...
        identities = sorted(
            {
                ...
                for document in documents
                if rendered_document_activates_registry(document)
            }
        )
        if identities:
            errors.append(
                f"active source {source_path!r} renders RegistryService {identities!r}"
            )
    return errors


def main() -> int:
    """Run all RegistryService GitOps policy checks."""
    errors: list[str] = []
    for environment in ENVIRONMENTS:
        try:
            errors.extend(validate_registry_overlay(environment))
        except (OSError, RuntimeError, yaml.YAMLError) as error:
            errors.append(str(error))
    errors.extend(validate_registry_applications_remain_disabled())
    if errors:
        print("RegistryService secret projection policy failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print("RegistryService secret projection policy passed for all environments.")
    return 0
